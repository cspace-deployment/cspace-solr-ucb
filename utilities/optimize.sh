time curl -S -s http://localhost:8983/solr/bampfa-internal/update --data '<optimize/>' -H 'Content-type:text/xml; charset=utf-8'
time curl -S -s http://localhost:8983/solr/bampfa-public/update --data '<optimize/>' -H 'Content-type:text/xml; charset=utf-8'
time curl -S -s http://localhost:8983/solr/botgarden-internal/update --data '<optimize/>' -H 'Content-type:text/xml; charset=utf-8'
time curl -S -s http://localhost:8983/solr/botgarden-propagations/update --data '<optimize/>' -H 'Content-type:text/xml; charset=utf-8'
time curl -S -s http://localhost:8983/solr/botgarden-public/update --data '<optimize/>' -H 'Content-type:text/xml; charset=utf-8'
